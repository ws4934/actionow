...
/**
 * Cloudflare R2 OSS 服务实现
 * <p>
 * R2 是 Cloudflare 提供的 S3 兼容对象存储服务
 * 使用独立的 R2 配置，与 AWS S3 配置完全分离
 * <p>
 * 支持功能：
 ...
 */
@Slf4j
public class R2OssService implements OssService {

    private final S3Client s3Client;
    private final S3Presigner s3Presigner;
    private final OssProperties ossProperties;
    private final OssProperties.R2Config r2Config;
    private final String bucketName;
    private final String endpoint;

    public R2OssService(OssProperties ossProperties) {
        this.ossProperties = ossProperties;
        this.r2Config = ossProperties.getR2();
        this.bucketName = r2Config.getBucket();
        this.endpoint = r2Config.getEndpoint();

        if (!StringUtils.hasText(endpoint)) {
            throw new IllegalStateException("Cloudflare R2 Account ID 未配置，无法初始化 R2 服务");
        }
        ...
        log.info("Cloudflare R2 客户端初始化完成: endpoint={}, bucket={}", endpoint, bucketName);
    }
    ...
    public String getUrl(String bucket, String objectName) {
        // 优先使用 R2 公开访问域名
        String publicDomain = r2Config.getPublicDomain();
