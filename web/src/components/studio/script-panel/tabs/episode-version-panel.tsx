...
/**
 * Episode Version Panel Component
 * Displays episode version history with view, compare and restore functionality
 */
...
import type { VersionInfoDTO, EpisodeVersionDetailDTO, VersionDiffDTO } from "@/lib/api/dto";
import { useWorkspace } from "@/components/providers/workspace-provider";
import { StatusBadge, VersionListSkeleton } from "../common";

interface EpisodeVersionPanelProps {
  episodeId: string;
  onRestore: () => void;
  onClose: () => void;
}

export function EpisodeVersionPanel({ episodeId, onRestore, onClose }: EpisodeVersionPanelProps) {
  const { currentWorkspaceId } = useWorkspace();
  const locale = useLocale();
  const [versions, setVersions] = useState<VersionInfoDTO[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedVersion, setSelectedVersion] = useState<EpisodeVersionDetailDTO | null>(null);
  ...
  useEffect(() => {
    const fetchVersions = async () => {
      if (!currentWorkspaceId || !episodeId) return;
      try {
        setIsLoading(true);
        const data = await projectService.getEpisodeVersions( episodeId);
        setVersions(data);
      } catch (err) {
        console.error("Failed to fetch episode versions:", err);
    ...
    fetchVersions();
  }, [currentWorkspaceId, episodeId]);

  const handleViewVersion = async (versionNumber: number) => {
    if (!currentWorkspaceId) return;
    try {
      const detail = await projectService.getEpisodeVersion( episodeId, versionNumber);
      setSelectedVersion(detail);
      setViewMode("detail");
    } catch (err) {
      console.error("Failed to fetch episode version detail:", err);
      toast.danger(getErrorFromException(err, locale));
    }
  };
  ...
  const handleCompare = async (v1: number, v2: number) => {
    ...
    try {
      ...
      const diff = await projectService.compareEpisodeVersions( episodeId, v1, v2);
      setDiffResult(diff);
      setViewMode("diff");
    } catch (err) {
      console.error("Failed to compare episode versions:", err);
      toast.danger(getErrorFromException(err, locale));
    }
  };
  ...
  const handleRestore = async (versionNumber: number) => {
    ...
    try {
      setIsRestoring(true);
      await projectService.restoreEpisodeVersion( episodeId, {
        versionNumber,
      ...
      console.error("Failed to restore episode version:", err);
      toast.danger(getErrorFromException(err, locale));
    } finally {
        ...
        {viewMode === "detail" && selectedVersion && (
          <div className="space-y-4 p-4">
            ...
            {selectedVersion.content && (
              <div className="rounded-xl bg-muted/5 p-4">
                <label className="text-[11px] font-medium uppercase text-muted/70">剧集内容</label>
